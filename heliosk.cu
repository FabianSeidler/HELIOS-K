...
__global__ void Voigt_texture_kernel(cudaTextureObject_t K2dtex, float *K_d, int
Nx, int Ny, int Nxtex, int Nytex, size_t pitch){ int idx = blockIdx.x *
blockDim.x + threadIdx.x; int idy = blockIdx.y * blockDim.y + threadIdx.y;
...
__global__ void Voigt_textureb_kernel(cudaTextureObject_t K2dtex, float *K_d,
int Nx, int Ny, int Nxtex, int Nytex, size_t pitch){ int idx = blockIdx.x *
blockDim.x + threadIdx.x; int idy = blockIdx.y * blockDim.y + threadIdx.y;
        ...
        if(idx < Nx && idy < Ny){
                ...
                float K = (1.0f - xx) * ( 1.0f - yy) * K00 + xx * (1.0f - yy) *
K10 + (1.0f - xx) * yy * K01 + xx * yy * K11;

                float *row = (float *)(((char *)K_d)+(idy*pitch));
                row[idx] = K;
//if(idy == 0) printf("%d %d %f %f | %f %f | %f %f %f %f %f\n", idx, idy, x
* 10.0f / Nx, y * 10.0f / Ny, xx, yy, K00, K10, K01, K11, K);

        }
}
// runs with manual biliniar interpolation
// texDescr.filterMode = cudaFilterModePoint;
__global__ void Voigt_b_kernel(float *K2d_d, float *K_d, int Nx, int Ny, int
Nxtex, int Nytex, size_t pitch){ int idx = blockIdx.x * blockDim.x +
threadIdx.x; int idy = blockIdx.y * blockDim.y + threadIdx.y;
...
// runs with manual biliniar interpolation
// texDescr.filterMode = cudaFilterModePoint;
__global__ void Voigt_bicubic_kernel(cudaTextureObject_t K2dtex, float *K_d, int
Nx, int Ny, int Nxtex, int Nytex, size_t pitch){ int idx = blockIdx.x *
blockDim.x + threadIdx.x; int idy = blockIdx.y * blockDim.y + threadIdx.y;
...
int main(int argc, char *argv[]) {
  ...
  double *K2d_h, *K2d_d;
  size_t pitch;
  //with pitch, the 2d memory is extendend in one dimension to set memory
  alignment, pitch is the new Nxtex K2d_h = (double*)malloc( Nxtex * Nytex *
  sizeof(double)); cudaMallocPitch((void **) &K2d_d, &pitch, Nxtex *
  sizeof(double), Nytex);
  //printf("%d %d %lu\n", Nxtex, Nytex, pitch);

  {
          ...
          Voigt_2d_kernel <<< dim3((Nxtex + 31) / 32, (Nytex + 31) / 32),
  dim3(32, 32, 1) >>> (a, b, c, K2d_d, Nxtex, Nytex, pitch, xMax, xMax);
          cudaMemcpy2D(K2d_h, Nxtex * sizeof(double), K2d_d, pitch, Nxtex *
  sizeof(double), Nytex, cudaMemcpyDeviceToHost);
  }
  // / *
  ...
  float *K2df_h, *K2df_d;
  size_t pitchf;
  //with pitchf, the 2d memory is extendend in one dimension to set memory
  alignment, pitchf is the new Nxtexf K2df_h = (float*)malloc( Nxtexf * Nytexf *
  sizeof(float)); cudaMallocPitch((void **) &K2df_d, &pitchf, Nxtexf *
  sizeof(float), Nytexf);
...

  {
          ...
          Voigt_2df_kernel <<< dim3((Nxtexf + 31) / 32, (Nytexf + 31) / 32),
  dim3(32, 32, 1) >>> (a, b, c, K2df_d, Nxtexf, Nytexf, pitchf, xMax, xMax);
          cudaMemcpy2D(K2df_h, Nxtexf * sizeof(float), K2df_d, pitchf, Nxtexf *
  sizeof(float), Nytexf, cudaMemcpyDeviceToHost);
  }
  / *
  ...
  * /
  ...
  //with pitch, the 2d memory is extendend in one dimension to set memory
  alignment, pitch is the new Nx cudaMallocPitch((void **) &K_d, &pitch, Nx *
  sizeof(float), Ny);


  for(int t = 0; t < 1; ++t){
          //Voigt_texture_kernel <<< dim3((Nx + 31) / 32, (Ny + 31) / 32),
  dim3(32, 32, 1) >>> (K2dtex, K_d, Nx, Ny, Nxtexf - 1, Nytexf - 1, pitch);
          //Voigt_textureb_kernel <<< dim3((Nx + 31) / 32, (Ny + 31) / 32),
  dim3(32, 32, 1) >>> (K2dtex, K_d, Nx, Ny, Nxtexf -1, Nytexf - 1, pitch);
          //Voigt_b_kernel <<< dim3((Nx + 31) / 32, (Ny + 31) / 32), dim3(32,
  32, 1) >>> (K2d_d, K_d, Nx, Ny, Nxtex - 1, Nytex - 1, pitch);
          Voigt_bicubic_kernel <<< dim3((Nx + 31) / 32, (Ny + 31) / 32),
  dim3(32, 32, 1) >>> (K2dtex, K_d, Nx, Ny, Nxtexf - 1, Nytexf - 1, pitch);
  }

  cudaMemcpy2D(K_h, Nx * sizeof(float), K_d, pitch, Nx * sizeof(float), Ny,
  ...
  for(int i = 0; i < Nx; ++i){
          for(int j = 0; j < Ny; ++j){
                  ...
                  if( x < xMax && y < yMax){
                          double diff = fabs(K2d_h[j * Nxtex + i] - K_h[j * Nx +
  i]); if(diff > 5.0e-7){ printf("%g %g %.15g %.15g %.15g\n", x, y, K2d_h[j *
  Nxtex + i], K_h[j * Nx + i], diff);
                          }
                  }
          }
  }
  ...
  */
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    for (int j = 0; j < devCount; ++j) {
      ...
      fprintf(infofile,
              "Name:%s, Major:%d, Minor:%d, Max threads per Block:%d, Max x "
              "dim:%d\n, #Multiprocessors:%d, Clock Rate:%d, Memory Clock "
              "Rate:%d, Global Memory:%lu, Shared memory per block: %lu\n",
              devProp.name, devProp.major, devProp.minor,
              devProp.maxThreadsPerBlock, devProp.maxThreadsDim[0],
              devProp.multiProcessorCount, devProp.clockRate,
              devProp.memoryClockRate,
              (long unsigned int)(devProp.totalGlobalMem),
              (long unsigned int)(devProp.sharedMemPerBlock));
    }
  }
  if (param.Nxb != 0) {
  ...
  if (param.useIndividualBins == 1) {
    ...
    if (param.doResampling > 0) {
      printf("Error: The resampling function is not supported for the bin-file "
             "option\n");
      return 0;
    }
    if (param.doTransmission > 0) {
      printf("Error: The transmission function is not supported for the "
             "bin-file option\n");
      return 0;
    }
  } else {
    for (int i = 0; i < param.nbins; ++i) {
      binBoundaries_h[i] = param.numin + i * (param.numax - param.numin) /
                                             ((double)(param.nbins));
    }
    binBoundaries_h[param.nbins] = param.numax;
  }
  cudaMemcpy(binBoundaries_d, binBoundaries_h,
             (param.nbins + 1) * sizeof(double), cudaMemcpyHostToDevice);
  ...
  if (param.useIndividualX == 0) {
    Nx = (int)((param.numax - param.numin) / param.dnu +
               0.5); //+ 0.5 to round correctly between double and int
    if ((param.numax - param.numin) / param.dnu + 0.5 >= 2147483647) {
      printf("Error: Nx too large, integer overflow. %d %g\n", Nx,
             (param.numax - param.numin) / param.dnu);
      return 0;
    }
    printf("%g %g %g %g\n", param.numax, param.numin, param.dnu,
           (param.numax - param.numin) / param.dnu + 0.5);
    param.Nxb = Nx / param.nbins;
    if (Nx % param.nbins != 0) {
      printf("Error: range cannot be divided evenly in bins. %d %d %g\n", Nx,
             param.nbins, Nx / ((double)(param.nbins)));
      return 0;
    }
  } else {
    ...
    if (param.nbins * param.Nxb >= 2147483647) {
      printf("Error: Nx too large, integer overflow. %d %g\n", Nx,
             (double)(param.nbins) * (double)(param.Nxb));
      return 0;
    }
    if (param.doResampling > 0) {
      printf("Error: The resampling function is not supported for unequal "
             "spacing option\n");
      return 0;
    }
    if (param.doTransmission > 0) {
      printf("Error: The transmission function is not supported for unequal "
             "spacing option\n");
      return 0;
    }
  }
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    if (param.Nxb < param.nC && i == 0) {
      printf("Number of points per bin smaller than the number of Chebyshev "
             "coefficients: Changed nC to %d\n",
             param.Nxb);
      fprintf(infofile,
              "Number of points per bin smaller than the number of Chebyshev "
              "coefficients: Changed nC to %d\n",
              param.Nxb);
      param.nC = param.Nxb;
    }
  ...
  // needed here already to get the cia.mass1. Initialize it again later in the
  // main species loop
  ciaSystem cia;
  if (param.useCia == 1) {
    Molecule m;
  ...
  for (int k = 0; k < param.nP * Nx; k += def_nthmax) {
    ...
    InitialK_kernel<<<(Nk + 511) / 512, 512>>>(K_d, param.nP * Nx, param.kmin,
                                               k);
  }
  for (int k = 0; k < def_KSn * Nx; k += def_nthmax) {
    int Nk = min(def_nthmax, def_KSn * Nx - k);
    // kmin must be here always zero, because the different streams are added
    // later to K_d
  ...
  for (int k = 0; k < Nx; k += def_nthmax) {
    ...
    setX_kernel<<<(Nk + 511) / 512, 512>>>(x_d, Nx, param.numin, param.dnu,
                                           param.Nxb, param.useIndividualX,
                                           binBoundaries_d, k);
  }
  cudaMemcpy(x_h, x_d, Nx * sizeof(double), cudaMemcpyDeviceToHost);
  for (int k = 0; k < Nx; k += def_nthmax) {
    int Nk = min(def_nthmax, Nx - k);
    binKey_kernel<<<(Nk + 511) / 512, 512>>>(
        binKey_d, Nx, param.Nxb, binBoundaries_d, param.nbins, param.numax, x_d,
        param.useIndividualX, k);
  }
  for (int k = 0; k < Nx; k += def_nthmax) {
    int Nk = min(def_nthmax, Nx - k);
    binIndex_kernel<<<(Nk + 511) / 512, 512>>>(binKey_d, binIndex_d, Nx,
                                               param.nbins, k);
  }
  cudaMemcpy(binIndex_h, binIndex_d, (param.nbins + 2) * sizeof(int),
  ...
  */
  ...
  for (int iSpecies = 0; iSpecies < param.nSpecies; ++iSpecies) {
    ...
    cudaHostAlloc((void **)&readBuffer_h,
                  def_rBs * readBufferSize * readBufferN * sizeof(double),
    ...
    cudaMalloc((void **)&readBuffer_d,
               def_maxlines * readBufferN * sizeof(double));
    ...
    // readBufferSize, readBufferN, m.NLmax,  def_rBs * readBufferSize *
    // readBufferN, m.NLmax * readBufferN);
    ...
    if (param.useCia == 1) {
      for (int iP = 0; iP < param.nP; ++iP) {
        ...
        cudaMemcpy(K_d + iP * Nx, K_h, Nx * sizeof(double),
    ...
    if (m.id > 0 && param.doStoreFullK >= 0) {
      ...
      if (param.cutMode == 0 && param.cut) {
        for (int fi = 0; fi < m.nFiles; ++fi) {
          if (m.fileLimit[fi] - param.cut <= param.numax)
            fi1 = fi + 1;
          else
            break;
        }
        for (int fi = m.nFiles - 1; fi >= 0; --fi) {
          if (m.fileLimit[fi + 1] + param.cut >= param.numin)
            fi0 = fi;
          else
            break;
        }
      ...
      for (fi = fi0; fi < fi1; ++fi) {
        ...
        if (fi == fi0) {
          ...
          printf(
              "Reading Line file %d of %d; part %lld of %lld with %d lines\n",
              fi, fi1 - 1, lPart,
              (m.NL[fi] + def_maxlines - 1) / def_maxlines - 1, NL);
          // **************************
          // Read the Line list
          // **************************
          if (param.dataBase < 2 || param.dataBase == 3) {
            ...
            for (int i = 0; i < NL; i += readBufferSize) {
              er = readFileExomol(L, NL, dataFile, readBuffer_h, readBuffer_d,
                                  readBufferSize, readBufferN, readBufferCount,
                                  vs, CStream);
        ...
        for (long long int iL = 0LL; iL < m.NL[fi]; iL += def_maxlines) {
          ...
          // start the loop around the Pressure values. only 1 iteration if no
          // Pressure file is given
          for (int iP = 0; iP < param.nP; ++iP) {
            ...
            if (param.dataBase < 2 || param.dataBase == 3) {
              ...
              for (int k = 0; k < NL; k += def_nthmax / 4) {
                ...
                if (Nk > 0) {
                  ...
                  // ***************************
                  if (param.dataBase == 2) {
                    L_kernelExomol<<<(Nk + 127) / 128, 128, 0,
                                     VStream[vs % def_KSn]>>>(
                        readBuffer_d, L.nu_d, L.S_d, L.EL_d, L.ialphaD_d, L.A_d,
                        L.vy_d, L.n_d, m.defaultL, m.defaultn, param.gammaF,
                        mass, param.T, Q, Abundance, Sscale, NL, k);
                  }
                  if (param.dataBase == 20) {
                    L_kernelExomolSuper<<<(Nk + 127) / 128, 128, 0,
                                          VStream[vs % def_KSn]>>>(
                        readBuffer_d, L.nu_d, L.S_d, L.ialphaD_d, L.vy_d, L.n_d,
                        m.defaultL, m.defaultn, param.gammaF, mass, param.T,
                        Abundance, Sscale, NL, k);
                  }
                  if (param.dataBase == 30) {
                    L_kernelKurucz<<<(Nk + 127) / 128, 128, 0,
                                     VStream[vs % def_KSn]>>>(
                        readBuffer_d, L.nu_d, L.S_d, L.EL_d, L.ialphaD_d, L.A_d,
                        L.vy_d, L.n_d, m.defaultL, m.defaultn, param.gammaF,
                        mass, param.T, Q, Abundance, Sscale, NL, k);
                  ...
                  // ***************************
                  if (param.dataBase != 20) {
                    Sf_kernel<<<(Nk + 127) / 128, 128, 0,
                                VStream[vs % def_KSn]>>>(
                        L.nu_d, L.S_d, L.A_d, L.vy_d, L.ialphaD_d, L.n_d,
                        L.EL_d, L.ID_d, NL, c1, T1, P_h[iP], k);
                  } else {
                    // 2 30 31 32
                    SfSuper_kernel<<<(Nk + 127) / 128, 128, 0,
                                     VStream[vs % def_KSn]>>>(
                        L.nu_d, L.S_d, L.vy_d, L.ialphaD_d, L.n_d, L.ID_d, NL,
                        T1, P_h[iP], k);
                  }
                }
                ++vs;
            ...
            if (param.dataBase < 2 || param.dataBase == 3) {
              ...
              for (int k = 0; k < NL; k += def_nthmax) {
                ...
                if (Nk > 0)
                  S2_kernel<<<(Nk + 127) / 128, 128>>>(
                      L.nu_d, L.S_d, L.A_d, L.vy_d, L.ialphaD_d, L.n_d,
                      L.delta_d, L.EL_d, L.ID_d, NL, param.T, P_h[iP], k);
              }
              /* // *************
                                      //uncoment this only when no Pressure file
                 is given
                                      //print number of lines per bin
                                      cudaMemcpy(L.nu_h, L.nu_d, NL *
                 sizeof(double), cudaMemcpyDeviceToHost); int nLb[param.nbins];
                                      for(int i = 0; i < param.nbins; ++i){
                                              nLb[i] = 0;
                                      }
                                      double binWidth = (param.numax -
                 param.numin) / ((double)(param.nbins)); printf("%g\n",
                 binWidth); for(int i = 0; i < NL; ++i){ int b = int(L.nu_h[i] /
                 binWidth); nLb[b] += 1;
                                      }
                                      for(int i = 0; i < param.nbins; ++i){
                                              printf("%d, ", nLb[i]);
            ...
            // print_kernel <<< 1, 1 >>> (L.nu_d, L.ialphaD_d, L.vy_d, L.ID_d,
            // 500, 0);

            // Sort the data along nu
            thrust::device_ptr<double> nu_dt =
                thrust::device_pointer_cast(L.nu_d);
            ...
            for (int k = 0; k < NL; k += def_nthmax) {
              if (Nk > 0)
                Copy_kernel<<<(Nk + 127) / 128, 128>>>(L.S_d, L.Sort_d, NL, k);
            }
            for (int k = 0; k < NL; k += def_nthmax) {
              if (Nk > 0)
                Sort_kernel<<<(Nk + 127) / 128, 128>>>(L.Sort_d, L.S_d, L.ID_d,
                                                       NL, k);
            }
            for (int k = 0; k < NL; k += def_nthmax) {
              if (Nk > 0)
                Copy_kernel<<<(Nk + 127) / 128, 128>>>(L.vy_d, L.Sort_d, NL, k);
            }
            for (int k = 0; k < NL; k += def_nthmax) {
              if (Nk > 0)
                Sort_kernel<<<(Nk + 127) / 128, 128>>>(L.Sort_d, L.vy_d, L.ID_d,
                                                       NL, k);
            }
            for (int k = 0; k < NL; k += def_nthmax) {
              if (Nk > 0)
                Copy_kernel<<<(Nk + 127) / 128, 128>>>(L.ialphaD_d, L.Sort_d,
                                                       NL, k);
            }
            for (int k = 0; k < NL; k += def_nthmax) {
              if (Nk > 0)
                Sort_kernel<<<(Nk + 127) / 128, 128>>>(L.Sort_d, L.ialphaD_d,
                                                       L.ID_d, NL, k);
            }
            // ********************************
            ...
            for (int k = 0; k < NL; k += def_nthmax) {
              int Nk = min(def_nthmax, NL - k);
              if (Nk > 0) {
                S3_kernel<<<(Nk + 127) / 128, 128>>>(
                    L.nu_d, L.S_d, L.S1_d, L.vy_d, L.ialphaD_d, L.Sf_d, L.S1f_d,
                    L.vyf_d, L.vcut2_d, L.va_d, L.vb_d, param.cut,
                    param.cutMode, param.profile, param.numin, param.dnu,
                    param.useIndividualX, NL, k);
                if (param.removePlinth == 1 && param.cut != 0.0) {
                  ...
                  Plinth_kernel<<<(Nk + 127) / 128, 128>>>(
                      L.S1f_d, L.Sf_d, L.vyf_d, L.vcut2_d, L.plinth_d, NL, a, b,
                      c, param.profile);
                  // printPlinth_kernel <<< (Nk + 127) / 128, 128 >>>
                  // (L.plinth_d, L.nu_d, NL);
                }
              }
            }
            ...
            // A
            nuLimits_kernel<<<nlLimitsA, min(def_nlA, 1024), 0,
                              nuLimitsStream[0]>>>(
                L.nu_d, L.ialphaD_d, L.vy_d, L.vcut2_d, L.nuLimitsA0_d,
                L.nuLimitsA1_d, param.numin, param.numax, def_nlA, NL,
                param.profile, 10);
            iiLimits_kernel<<<(nlLimitsA + 127) / 128, 128, 0,
                              nuLimitsStream[0]>>>(
                L.nuLimitsA0_d, L.nuLimitsA1_d, L.iiLimitsA0_d, L.iiLimitsA1_d,
                binBoundaries_d, nlLimitsA, param.numin, param.dnu, Nx,
                param.useIndividualX, param.nbins, param.Nxb, 10);

            iiLimitsMax_kernel<512><<<1, 512>>>(L.iiLimitsA0_d, L.iiLimitsA1_d,
                                                L.iiLimitsAT_d, Nx, nlLimitsA);

            if (param.profile == 1) { // only for voigt profiles
              // AL
              nuLimits_kernel<<<nlLimitsA, min(def_nlA, 1024), 0,
                                nuLimitsStream[1]>>>(
                  L.nu_d, L.ialphaD_d, L.vy_d, L.vcut2_d, L.nuLimitsAL0_d,
                  L.nuLimitsAL1_d, param.numin, param.numax, def_nlA, NL,
                  param.profile, 11);
              iiLimits_kernel<<<(nlLimitsA + 127) / 128, 128, 0,
                                nuLimitsStream[1]>>>(
                  L.nuLimitsAL0_d, L.nuLimitsAL1_d, L.iiLimitsAL0_d,
                  L.iiLimitsAL1_d, binBoundaries_d, nlLimitsA, param.numin,
                  param.dnu, Nx, param.useIndividualX, param.nbins, param.Nxb,
                  11);

              iiLimitsMax_kernel<512>
                  <<<1, 512>>>(L.iiLimitsAL0_d, L.iiLimitsAL1_d,
                               L.iiLimitsALT_d, Nx, nlLimitsA);

              // AR
              nuLimits_kernel<<<nlLimitsA, min(def_nlA, 1024), 0,
                                nuLimitsStream[2]>>>(
                  L.nu_d, L.ialphaD_d, L.vy_d, L.vcut2_d, L.nuLimitsAR0_d,
                  L.nuLimitsAR1_d, param.numin, param.numax, def_nlA, NL,
                  param.profile, 12);
              iiLimits_kernel<<<(nlLimitsA + 127) / 128, 128, 0,
                                nuLimitsStream[2]>>>(
                  L.nuLimitsAR0_d, L.nuLimitsAR1_d, L.iiLimitsAR0_d,
                  L.iiLimitsAR1_d, binBoundaries_d, nlLimitsA, param.numin,
                  param.dnu, Nx, param.useIndividualX, param.nbins, param.Nxb,
                  12);

              iiLimitsMax_kernel<512>
                  <<<1, 512>>>(L.iiLimitsAR0_d, L.iiLimitsAR1_d,
                               L.iiLimitsART_d, Nx, nlLimitsA);
              // B
              nuLimits_kernel<<<nlLimitsB, min(def_nlB, 1024), 0,
                                nuLimitsStream[3]>>>(
                  L.nu_d, L.ialphaD_d, L.vy_d, L.vcut2_d, L.nuLimitsB0_d,
                  L.nuLimitsB1_d, param.numin, param.numax, def_nlB, NL,
                  param.profile, 20);
              iiLimits_kernel<<<(nlLimitsB + 127) / 128, 128, 0,
                                nuLimitsStream[3]>>>(
                  L.nuLimitsB0_d, L.nuLimitsB1_d, L.iiLimitsB0_d,
                  L.iiLimitsB1_d, binBoundaries_d, nlLimitsB, param.numin,
                  param.dnu, Nx, param.useIndividualX, param.nbins, param.Nxb,
                  20);

              iiLimitsMax_kernel<512>
                  <<<1, 512>>>(L.iiLimitsB0_d, L.iiLimitsB1_d, L.iiLimitsBT_d,
                               Nx, nlLimitsB);
              // C
              nuLimits_kernel<<<nlLimitsC, min(def_nlC, 1024), 0,
                                nuLimitsStream[4]>>>(
                  L.nu_d, L.ialphaD_d, L.vy_d, L.vcut2_d, L.nuLimitsC0_d,
                  L.nuLimitsC1_d, param.numin, param.numax, def_nlC, NL,
                  param.profile, 30);
              iiLimits_kernel<<<(nlLimitsC + 127) / 128, 128, 0,
                                nuLimitsStream[4]>>>(
                  L.nuLimitsC0_d, L.nuLimitsC1_d, L.iiLimitsC0_d,
                  L.iiLimitsC1_d, binBoundaries_d, nlLimitsC, param.numin,
                  param.dnu, Nx, param.useIndividualX, param.nbins, param.Nxb,
                  30);

              iiLimitsMax_kernel<512>
                  <<<1, 512>>>(L.iiLimitsC0_d, L.iiLimitsC1_d, L.iiLimitsCT_d,
                               Nx, nlLimitsC);
            }

            cudaEventRecord(iiLimitsEvent);
            cudaEventSynchronize(iiLimitsEvent);
            iiLimitsCheck<<<(nlLimitsA + 127) / 128, 128>>>(
                L.iiLimitsA0_d, L.iiLimitsA1_d, L.iiLimitsAL0_d,
                L.iiLimitsAL1_d, L.iiLimitsAR0_d, L.iiLimitsAR1_d, nlLimitsA);
            cudaEventRecord(iiLimitsEvent);
            cudaEventSynchronize(iiLimitsEvent);
            ...
            printf("A Limits %lld %lld | %lld\n", L.iiLimitsAT_m[0],
                   L.iiLimitsAT_m[1], nTA);
            printf("AL Limits %lld %lld | %lld\n", L.iiLimitsALT_m[0],
                   L.iiLimitsALT_m[1], nTAL);
            printf("AR Limits %lld %lld | %lld\n", L.iiLimitsART_m[0],
                   L.iiLimitsART_m[1], nTAR);
            printf("B Limits %lld %lld | %lld\n", L.iiLimitsBT_m[0],
                   L.iiLimitsBT_m[1], nTB);
            printf("C Limits %lld %lld | %lld\n", L.iiLimitsCT_m[0],
                   L.iiLimitsCT_m[1], nTC);

            if (nTA > 0) {
              cudaMemcpyAsync(L.iiLimitsA0_h, L.iiLimitsA0_d,
                              nlLimitsA * sizeof(long long int),
                              cudaMemcpyDeviceToHost, nuLimitsStream[0]);
              cudaMemcpyAsync(L.iiLimitsA1_h, L.iiLimitsA1_d,
                              nlLimitsA * sizeof(long long int),
                              cudaMemcpyDeviceToHost, nuLimitsStream[0]);
            }
            if (nTAL > 0) {
              cudaMemcpyAsync(L.iiLimitsAL0_h, L.iiLimitsAL0_d,
                              nlLimitsA * sizeof(long long int),
                              cudaMemcpyDeviceToHost, nuLimitsStream[1]);
              cudaMemcpyAsync(L.iiLimitsAL1_h, L.iiLimitsAL1_d,
                              nlLimitsA * sizeof(long long int),
                              cudaMemcpyDeviceToHost, nuLimitsStream[1]);
            }
            if (nTAR > 0) {
              cudaMemcpyAsync(L.iiLimitsAR0_h, L.iiLimitsAR0_d,
                              nlLimitsA * sizeof(long long int),
                              cudaMemcpyDeviceToHost, nuLimitsStream[2]);
              cudaMemcpyAsync(L.iiLimitsAR1_h, L.iiLimitsAR1_d,
                              nlLimitsA * sizeof(long long int),
                              cudaMemcpyDeviceToHost, nuLimitsStream[2]);
            }

            if (nTB > 0) {
              cudaMemcpyAsync(L.iiLimitsB0_h, L.iiLimitsB0_d,
                              nlLimitsB * sizeof(long long int),
                              cudaMemcpyDeviceToHost, nuLimitsStream[3]);
              cudaMemcpyAsync(L.iiLimitsB1_h, L.iiLimitsB1_d,
                              nlLimitsB * sizeof(long long int),
                              cudaMemcpyDeviceToHost, nuLimitsStream[3]);
            }

            if (nTC > 0) {
              cudaMemcpyAsync(L.iiLimitsC0_h, L.iiLimitsC0_d,
                              nlLimitsC * sizeof(long long int),
                              cudaMemcpyDeviceToHost, nuLimitsStream[4]);
              cudaMemcpyAsync(L.iiLimitsC1_h, L.iiLimitsC1_d,
                              nlLimitsC * sizeof(long long int),
                              cudaMemcpyDeviceToHost, nuLimitsStream[4]);
            }
            ...
            int fii = fi;
            if ((iL < m.NL[fi] - def_maxlines || fi < fi1 - 1) &&
                iP == param.nP - 1) {
              // read the next line file while calculating the K kernels of the
              // current file
              //  **************************read iL + 1
              ...
              cudaEventRecord(ReadStart);
              printf("Reading Line file %d of %d; part %lld of %lld with %d "
                     "lines\n",
                     fii, fi1 - 1, lPart,
                     (m.NL[fii] + def_maxlines - 1) / def_maxlines - 1, NL);
              readBufferCount = 0;
              rbvs = 0;
            }
            ...
            */
            if (nTA > 0) {
              ...
              const int nntt = 128;
              for (int il = 0; il < NL;
                   il += def_KSn * def_nlB) { // loop over lines
                // *************************************
                // Call A Line kernels
                ntA = Line6A_Call(L, param, KS_d, x_d, il, NL, nntt, nkA, Nx,
                                  tuneStream[0], 10, 1);
                // *************************************
                if (param.doTuning == 1) {
                  if (ntA > 0 && ntA < 0.6 * ntAOld || ntA > 1.6 * ntAOld) {
                    ...
                    for (int k = 0; k < 2; ++k) {
                      ...
                      for (int j = 0; j < 8; ++j) {
                        ...
                        Line6A_Call(L, param, KS_d, x_d, il, NL, nntt, nkt, Nx,
                                    tuneStream[1], 10, 0);

                        cudaEventRecord(TuneStop);
                        cudaEventSynchronize(TuneStop);
                        cudaEventElapsedTime(&milliseconds, TuneStart,
                                             TuneStop);
              ...
              if ((iL < m.NL[fi] - def_maxlines || fi < fi1 - 1) &&
                ...
                if (param.dataBase == 2 || param.dataBase >= 20) {
                  ...
                  for (int i = readBufferCount; i < NL1; i += readBufferSize) {
                    // check if the A kernels have finished, otherwise use host
                    // to read more data
                    ...
                    er = readFileExomol(L, NL1, dataFile, readBuffer_h,
                                        readBuffer_d, readBufferSize,
                                        readBufferN, readBufferCount, rbvs,
                                        CStream);
                    readBufferCount += readBufferSize;
                    ++rbvs;
                  }
            ...
            if (param.profile == 1) { // only for voigt profiles
              if (nTAL > 0) {
                ...
                for (int il = 0; il < NL;
                  ...
                  ntAL = Line6A_Call(L, param, KS_d, x_d, il, NL, nntt, nkAL,
                                     Nx, tuneStream[0], 11, 1);
                  // *************************************
                  if (param.doTuning == 1) {
                    if (ntAL > 0 && ntAL < 0.6 * ntALOld ||
                        ntAL > 1.6 * ntALOld) {
                      ntALOld = ntAL;
                      int nkt;
                      int nktt = nkAL;
                      ...
                      for (int k = 0; k < 2; ++k) {
                        ...
                        for (int j = 0; j < 8; ++j) {
                          ...
                          Line6A_Call(L, param, KS_d, x_d, il, NL, nntt, nkt,
                                      Nx, tuneStream[1], 11, 0);
                ...
                if ((iL < m.NL[fi] - def_maxlines || fi < fi1 - 1) &&
                  ...
                  if (param.dataBase == 2 || param.dataBase >= 20) {
                    // 2 20 30 31 32
                    for (int i = readBufferCount; i < NL1;
                         i += readBufferSize) {
                      // check if the AL kernels have finished, otherwise use
                      // host to read more data
              ...
              if (nTAR > 0) {
                ...
                for (int il = 0; il < NL;
                  ...
                  ntAR = Line6A_Call(L, param, KS_d, x_d, il, NL, nntt, nkAR,
                                     Nx, tuneStream[0], 12, 1);
                  // *************************************
                  if (param.doTuning == 1) {
                    if (ntAR > 0 && ntAR < 0.6 * ntAROld ||
                        ntAR > 1.6 * ntAROld) {
                      ntAROld = ntAR;
                      int nkt;
                      int nktt = nkAR;
                ...
                if ((iL < m.NL[fi] - def_maxlines || fi < fi1 - 1) &&
                  ...
                  if (param.dataBase == 2 || param.dataBase >= 20) {
                    ...
                    for (int i = readBufferCount; i < NL1;
                         i += readBufferSize) {
                      // check if the AR kernels have finished, otherwise use
                      // host to read more data
              ...
              if (nTB > 0) {
                ...
                for (int il = 0; il < NL;
                  ...
                  // *************************************
                  // Call B Line kernels
                  ntB = Line6B_Call(L, param, KS_d, x_d, il, NL, nntt2, nkB, Nx,
                                    tuneStream[0], 1);
                  // *************************************
                    ...
                    if (ntB > 0 && ntB < 0.6 * ntBOld || ntB > 1.6 * ntBOld) {
                      ...
                      for (int k = 0; k < 2; ++k) {
                        ...
                        for (int j = 0; j < 8; ++j) {
                          ...
                          Line6B_Call(L, param, KS_d, x_d, il, NL, nntt2, nkt,
                                      Nx, tuneStream[1], 0);
                ...
                if ((iL < m.NL[fi] - def_maxlines || fi < fi1 - 1) &&
                  ...
                  if (param.dataBase == 2 || param.dataBase >= 20) {
                    ...
                    for (int i = readBufferCount; i < NL1;
                         i += readBufferSize) {
                      // check if the B kernels have finished, otherwise use
                      // host to read more data
              ...
              if (nTC > 0) {
                ...
                float c = (float)(2.0 * a / M_PI);
                for (int il = 0; il < NL;
                     il += def_KSn * def_nlC) { // loop over lines
                  // *************************************
                  // Call C Line kernels
                  ntC = Line6C_Call(L, param, KS_d, x_d, il, NL, nntt3, nkC, Nx,
                                    a, b, c, tuneStream[0], 1);
                  // *************************************
                    ...
                    if (ntC > 0 && ntC < 0.6 * ntCOld || ntC > 1.6 * ntCOld) {
                      ...
                      for (int k = 0; k < 2; ++k) {
                        ...
                        for (int j = 0; j < 8; ++j) {
                          ...
                          Line6C_Call(L, param, KS_d, x_d, il, NL, nntt3, nkt,
                                      Nx, a, b, c, tuneStream[1], 0);
            ...
            if ((iL < m.NL[fi] - def_maxlines || fi < fi1 - 1) &&
              ...
              if (param.dataBase == 2 || param.dataBase >= 20) {
                ...
                for (int i = readBufferCount; i < NL1; i += readBufferSize) {
                  er = readFileExomol(L, NL1, dataFile, readBuffer_h,
                                      readBuffer_d, readBufferSize, readBufferN,
                                      readBufferCount, rbvs, CStream);
                  readBufferCount += readBufferSize;
                  ++rbvs;
                }
              }
            ...
            if ((iL < m.NL[fi] - def_maxlines || fi < fi1 - 1) &&
              ...
              if (param.dataBase < 2 || param.dataBase == 3) {
                // 0 1 3
                er = readFile(param, m, part, L, param.qalphaL, NL1, dataFile,
                              Sscale, meanMass);
                if (er == 0) {
                  return 0;
                }
            ...
            // set KS_d to zero
            AddKStreams_kernel<<<(Nx + 511) / 512, 512>>>(K_d + iP * Nx, KS_d,
                                                          def_KSn, Nx);
            printf("Add streams B\n");

            // *************************************
            // synchronize here only if no more data has to be read from the
            // disk. otherwise read data before synchronization
  ...
  if (param.doStoreFullK == 1) {
    ...
    for (int iP = 0; iP < param.nP; ++iP) {
      cudaMemcpy(K_h, K_d + iP * Nx, Nx * sizeof(double),
                 cudaMemcpyDeviceToHost);
      for (int j = 0; j < Nx; ++j) {
        ...
        if (param.nP == 1) {
          ...
          fprintf(OutFile, "%.20g %.20g %.20g %.20g\n", x_h[j],
                  K_h[j] * unitScale, param.T, P_h[iP]);
        }
      }
      fprintf(OutFile, "\n\n");
    }
  ...
  if (param.doMean > 0) {
    ...
    for (int iP = 0; iP < param.nP; ++iP) {
      ...
      printf("\n\n");
      */
      IntegrateMean_kernel<512><<<4, 512>>>(Pmn_d, Rmn_d, x_d, K_d + iP * Nx,
                                            means_d, Nx, param.useIndividualX);
      double sigma = 2.0 * def_kB * def_kB * def_kB * def_kB /
                     (def_h * def_h * def_h * def_c * def_c * 15.0) * M_PI *
                     M_PI * M_PI * M_PI * M_PI;
  ...
  // printf("***MEMORY %g %g %g\n", (double)(free_byte), (double)(total_byte),
  // (double)(total_byte) - (double)(free_byte));
  int *Nxmin_h, *Nxmin_d;
  ...
  if (param.doResampling > 0) {
    ...
    Vandermonde_kernel<<<(param.Nxb + 511) / 512, 512>>>(
        V_d, (double)(param.Nxb), param.nC);
    ...
    for (int iP = 0; iP < param.nP; ++iP) {
      ...
      findCut_kernel<<<(Nx + 511) / 512, 512>>>(
          K_d + iP * Nx, Nx, param.Nxb, param.kmin, Nxmin_d, param.nbins);
      rescale_kernel<512><<<param.nbins, 512>>>(Nxmin_d, K_d + iP * Nx, K2_d,
                                                param.Nxb, param.kmin, 1);
      /*
      ...
      */
      copyK2_kernel<512>
          <<<param.nbins, 512>>>(Nxmin_d, K_d + iP * Nx, K2_d, param.Nxb);
      cudaMemcpy(Nxmin_h, Nxmin_d, param.nbins * sizeof(int),
                 cudaMemcpyDeviceToHost);

      lnK_kernel<<<(Nx + 511) / 512, 512>>>(K_d + iP * Nx, Nx, unitScale);
      leastSquare_kernel<512><<<param.nbins, 512>>>(
          V_d, C_d, D_d, K_d + iP * Nx, param.Nxb, param.nC);

      for (int i = 0; i < param.nbins; ++i) {
        int il = i * param.Nxb;
        cudaMemcpy(K_h + il, K_d + il + iP * Nx, param.nC * sizeof(double),
                   cudaMemcpyDeviceToHost);

        fprintf(Out3File, "%.20g %.20g ", param.kmin,
                fmin(Nxmin_h[i] / ((double)(param.Nxb - 1)), 1.0));
        for (int ic = 0; ic < param.nC; ++ic) {
          if (Nxmin_h[i] != param.Nxb)
            fprintf(Out3File, "%.20g ", K_h[il + ic]);
          else
            fprintf(Out3File, "0.0 ");
        }
        if (param.nP > 1) {
            ...
            if (param.useOutputEdges == 0) {
              if (param.nP == 1) {
                ...
                fprintf(Out2File, "%g %.20g %g %g %d\n", y,
                        K_h[j + il] * unitScale, param.T, P_h[iP], j);
              }
            } else {
              ...
              if (y <= edge && edge <= y1 && iedge < param.nedges) {
                if (param.nP == 1) {
                  if (iedge > 0)
                    fprintf(Out2File, "%g %.20g\n",
                            0.5 * (edge + outputEdges_h[iedge - 1]),
                            sedge / ((double)(nedge)));
                } else {
                  if (iedge > 0)
                    fprintf(Out2File, "%g %.20g %g %g %d\n",
                            0.5 * (edge + outputEdges_h[iedge - 1]),
                            sedge / ((double)(nedge)), param.T, P_h[iP],
                            iedge - 1);
                }
                ++iedge;
                nedge = 0;
          ...
          if (param.useOutputEdges == 0) {
            if (param.nP == 1) {
              ...
              fprintf(Out2File, "%g %.20g %.20g %.20g %d\n", y,
                      K_h[i] * unitScale, param.T, P_h[iP], j);
            }
          } else {
            double edge = outputEdges_h[iedge];
            ++nedge;
  ...
  if (param.doTransmission > 0) {
    ...
    for (int iP = 0; iP < param.nP; ++iP) {
      ...
      Integrate_kernel<512>
          <<<param.nbins, 512>>>(K_d + iP * Nx, Tr_d, param.Nxb, param.nTr,
                                 param.dTr, Nxmin_d, param.kmin);
      cudaMemcpy(Tr_h, Tr_d, param.nbins * param.nTr * sizeof(double),
                 cudaMemcpyDeviceToHost);
      for (int i = 0; i < param.nbins; ++i) {
        for (int j = 0; j < param.nTr; ++j) {
          ...
          if (param.nP == 1) {
            ...
            fprintf(Out3File, "%.20g %.20g %.20g %.20g %d\n", m,
                    Tr_h[i * param.nTr + j], param.T, P_h[iP], j);
          }
        }
        if (param.doTransmission == 1) {
          fprintf(Out3File, "\n\n");
