...
// The ids must be adapted in order to be consistent with the line list files in
// HITRAN The parameters are: m.id: Id of molecule m.NL: Number of Lines in the
// Molecule file m.nISO: Number of Isotopologues in the Molecule file The last
// line specifies the name of the Molecule file.
...
int Init(Molecule &m, Param &param, char (*qFilename)[160]) {
  ...
  */
  ...
  if (strcmp(sp, "Number of Isotopes ") != 0 &&
      strcmp(sp, "Number of Isotopologues ") != 0) {
  ...
  if (strcmp(sp, "Number of columns in partition File =") != 0) {
    printf(
        "Error in molecule.param file, Number of columns in partition File\n");
    return 0;
  }
  ...
  for (int i = 0; i < m.nFiles; ++i) {
    if (param.dataBase == 0) {
      // Hitran or HITEMP
      if (m.nFiles > 1) {
        sprintf(m.dataFilename[i], "%s%s_%05d-%05d.", param.path, m.mName,
                m.fileLimit[i], m.fileLimit[i + 1]);
      } else {
    ...
    if (param.dataBase == 2) {
      ...
      // Kurucz Molecules
      if (m.nFiles > 1) {
        sprintf(m.dataFilename[i], "%s%s__%05d-%05d.", param.path, m.mName,
                m.fileLimit[i], m.fileLimit[i + 1]);
      } else {
        sprintf(m.dataFilename[i], "%s%s.", param.path, m.mName);
      }
    ...
    if (param.dataBase == 3) {
      // IAO (CDSD)
      if (m.nFiles > 1) {
        sprintf(m.dataFilename[i], "%s%s__%05d_%05d.", param.path, m.mName,
                m.fileLimit[i], m.fileLimit[i + 1]);
      } else {
        sprintf(m.dataFilename[i], "%s%s.", param.path, m.mName);
      }
  ...
  if (strcmp(sp, "Number of columns in transition files =") != 0) {
    printf(
        "Error in species.param file, Number of columns in transition files\n");
    return 0;
  }
  ...
  fgets(sp, 55, pFile);
  if (strcmp(sp, "Default value of Lorentzian half-width for all lines =") !=
      0) {
    printf("Error in species.param file, Default value of Lorentzian "
           "half-width for all lines\n");
    return 0;
  }
  ...
  fgets(sp, 54, pFile);
  if (strcmp(sp, "Default value of temperature exponent for all lines =") !=
      0) {
    printf("Error in species.param file, Default value of temperature exponent "
           "for all lines\n");
    return 0;
  }
...
int InitCia(Molecule &m, ciaSystem &cia, Param param) {
  ...
  } else if (strcmp(param.ciaSystem, "CO2-H2") == 0) {
    cia.Nsets = 4;
    sprintf(cia.dataFilename, "%s%s", param.path, "CO2-H2_2018.cia");
    cia.mass1 = 2.0 * 1.00794; // mass of H2 in g / mol
  } else if (strcmp(param.ciaSystem, "CO2-CO2") == 0) {
    cia.Nsets = 20;
    sprintf(cia.dataFilename, "%s%s", param.path, "CO2-CO2_2018.cia");
    cia.mass1 = 44.009; // mass of CO2 in g / mol
  } else if (strcmp(param.ciaSystem, "CO2-He") == 0) {
    cia.Nsets = 1;
    sprintf(cia.dataFilename, "%s%s", param.path, "CO2-CO2_2018.cia");
    cia.mass1 = 4.002602; // mass of He in g / mol
  } else {
