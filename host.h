/*
 * This file is part of HELIOS-K.
 *
 * HELIOS-K is free software: you can redistribute it and/or modify
 * it under the terms of the GNU General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * HELIOS-K is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 *
 * You should have received a copy of the GNU General Public License
 * along with HELIOS-K.  If not, see <http://www.gnu.org/licenses/>.
 *
 * Copyright (C) 2015-2018 Simon L. Grimm, 2025 Fabian L. Seidler
 */
...
// March 2018
//  *****************************************
__host__ int readPartition(Param &param, char (*qFilename)[160],
                           Partition &part, double T, Molecule &m) {
  ...
  for (int i = 0; i < m.nISO; ++i) {
    ...
    for (int j = 0; j < 100000; ++j) {
      ...
      if (T0 == T1 && T0 < T && j > 0) {
        printf("Error: partition function not valid for given temperature. T0: "
               "%g, T: %g\n",
               T0, T);
        return 0;
      }
      if (T1 > T && j == 0) {
        printf("Error: partition function not valid for given temperature. T1: "
               "%g, T: %g\n",
               T1, T);
        return 0;
      }
...
__host__ int read_parameters(Param &param, char *paramFilename, int argc,
                             char *argv[]) {
  ...
  if (strcmp(param.subLorentzianFilename, "-") != 0) {
    ...
    er = fscanf(sLfile, "%s %f %f %f", b, &sLchi_h[3], &sLchi_h[4],
                &sLchi_h[5]); // B1
    er = fscanf(sLfile, "%s %f %f %f", b, &sLchi_h[6], &sLchi_h[7],
                &sLchi_h[8]); // B2
    er = fscanf(sLfile, "%s %f %f %f", b, &sLchi_h[9], &sLchi_h[10],
                &sLchi_h[11]); // B3
    // read range
...
__host__ int readSpeciesFile(Param &param, char **SpeciesN_h,
                             double *SpeciesA_h) {
  FILE *Speciesfile;
  Speciesfile = fopen(param.SpeciesFilename, "r");
  int er;
...
// January 2015
// *******************************************************************
__host__ int readFile(Param param, Molecule &m, Partition &part, Line &L,
                      double qalphaL, int NL, FILE *dataFile, double Sscale,
                      double meanMass) {
...
// August 2016
// *******************************************************************
__host__ int readFileExomol(Line &L, int NL, FILE *dataFile,
                            double *readBuffer_h, double *readBuffer_d,
                            int readBufferSize, int readBufferN, int i, int vs,
                            cudaStream_t *Stream) {
  ...
  // printf("read %d %d %d %g\n", i, readBufferN * Size, bSwap,
  // readBuffer_h[0]); cudaMemcpy(readBuffer_d + i * readBufferN, readBuffer_h +
  // bSwap, Size * readBufferN * sizeof(double), cudaMemcpyHostToDevice);
  cudaMemcpyAsync(readBuffer_d + i * readBufferN, readBuffer_h + bSwap,
                  Size * readBufferN * sizeof(double), cudaMemcpyHostToDevice,
                  Stream[vs % def_rBs]);
...
/*=============================================================*\
Reads a HITRAN formatted CIA-file (https://hitran.org/cia/) and
calculates the opacity generated by collision-induced-absorption
(CIA). The implementation for the opacity follows Richard et al 2012
(doi:10.1016/j.jqsrt.2011.11.004).

\*=============================================================*/
__host__ int readCiaFile(Param param, ciaSystem cia, double *x_h, double *K_h,
                         int Nx, double T, double P) {
  ...
  // These chars refer to the sections of the header of the CIA-files in HITRAN,
  // as shown by Richard et al 2012, Fig.1
  //
  //	skip	- dumping ground for chemical symbol (20 chars) and comments
  //	c1		- minimum wavenumber of a set
  //	c2		- maximum wavenumber of a set
  //	c3		- number of points (?)
  //	c4		- temperature [K]
  //	c5		- pressure [Torr]
  char skip[160];
...
__host__ void Alloc_Line(Line &L, Molecule &m, Param param) {
  ...
  cudaHostAlloc((void **)&L.iiLimitsA0_h,
                (n + def_nlA - 1) / def_nlA * sizeof(long long int),
                cudaHostAllocDefault);
  cudaHostAlloc((void **)&L.iiLimitsA1_h,
                (n + def_nlA - 1) / def_nlA * sizeof(long long int),
                cudaHostAllocDefault);
  cudaHostAlloc((void **)&L.iiLimitsAL0_h,
                (n + def_nlA - 1) / def_nlA * sizeof(long long int),
                cudaHostAllocDefault);
  cudaHostAlloc((void **)&L.iiLimitsAL1_h,
                (n + def_nlA - 1) / def_nlA * sizeof(long long int),
                cudaHostAllocDefault);
  cudaHostAlloc((void **)&L.iiLimitsAR0_h,
                (n + def_nlA - 1) / def_nlA * sizeof(long long int),
                cudaHostAllocDefault);
  cudaHostAlloc((void **)&L.iiLimitsAR1_h,
                (n + def_nlA - 1) / def_nlA * sizeof(long long int),
                cudaHostAllocDefault);
  cudaHostAlloc((void **)&L.iiLimitsB0_h,
                (n + def_nlB - 1) / def_nlB * sizeof(long long int),
                cudaHostAllocDefault);
  cudaHostAlloc((void **)&L.iiLimitsB1_h,
                (n + def_nlB - 1) / def_nlB * sizeof(long long int),
                cudaHostAllocDefault);
  cudaHostAlloc((void **)&L.iiLimitsC0_h,
                (n + def_nlC - 1) / def_nlC * sizeof(long long int),
                cudaHostAllocDefault);
  cudaHostAlloc((void **)&L.iiLimitsC1_h,
                (n + def_nlC - 1) / def_nlC * sizeof(long long int),
                cudaHostAllocDefault);
  ...
  cudaMalloc((void **)&L.nuLimitsA0_d,
             (n + def_nlA - 1) / def_nlA * sizeof(double));
  cudaMalloc((void **)&L.nuLimitsA1_d,
             (n + def_nlA - 1) / def_nlA * sizeof(double));
  cudaMalloc((void **)&L.nuLimitsAL0_d,
             (n + def_nlA - 1) / def_nlA * sizeof(double));
  cudaMalloc((void **)&L.nuLimitsAL1_d,
             (n + def_nlA - 1) / def_nlA * sizeof(double));
  cudaMalloc((void **)&L.nuLimitsAR0_d,
             (n + def_nlA - 1) / def_nlA * sizeof(double));
  cudaMalloc((void **)&L.nuLimitsAR1_d,
             (n + def_nlA - 1) / def_nlA * sizeof(double));
  cudaMalloc((void **)&L.nuLimitsB0_d,
             (n + def_nlB - 1) / def_nlB * sizeof(double));
  cudaMalloc((void **)&L.nuLimitsB1_d,
             (n + def_nlB - 1) / def_nlB * sizeof(double));
  cudaMalloc((void **)&L.nuLimitsC0_d,
             (n + def_nlC - 1) / def_nlC * sizeof(double));
  cudaMalloc((void **)&L.nuLimitsC1_d,
             (n + def_nlC - 1) / def_nlC * sizeof(double));

  cudaMalloc((void **)&L.iiLimitsA0_d,
             (n + def_nlA - 1) / def_nlA * sizeof(long long int));
  cudaMalloc((void **)&L.iiLimitsA1_d,
             (n + def_nlA - 1) / def_nlA * sizeof(long long int));
  cudaMalloc((void **)&L.iiLimitsAL0_d,
             (n + def_nlA - 1) / def_nlA * sizeof(long long int));
  cudaMalloc((void **)&L.iiLimitsAL1_d,
             (n + def_nlA - 1) / def_nlA * sizeof(long long int));
  cudaMalloc((void **)&L.iiLimitsAR0_d,
             (n + def_nlA - 1) / def_nlA * sizeof(long long int));
  cudaMalloc((void **)&L.iiLimitsAR1_d,
             (n + def_nlA - 1) / def_nlA * sizeof(long long int));
  cudaMalloc((void **)&L.iiLimitsB0_d,
             (n + def_nlB - 1) / def_nlB * sizeof(long long int));
  cudaMalloc((void **)&L.iiLimitsB1_d,
             (n + def_nlB - 1) / def_nlB * sizeof(long long int));
  cudaMalloc((void **)&L.iiLimitsC0_d,
             (n + def_nlC - 1) / def_nlC * sizeof(long long int));
  cudaMalloc((void **)&L.iiLimitsC1_d,
             (n + def_nlC - 1) / def_nlC * sizeof(long long int));

  // mapped memory
  cudaHostAlloc((void **)&L.iiLimitsAT_m, 2 * sizeof(unsigned long long int),
                cudaHostAllocDefault);
  cudaHostAlloc((void **)&L.iiLimitsALT_m, 2 * sizeof(unsigned long long int),
                cudaHostAllocDefault);
  cudaHostAlloc((void **)&L.iiLimitsART_m, 2 * sizeof(unsigned long long int),
                cudaHostAllocDefault);
  cudaHostAlloc((void **)&L.iiLimitsBT_m, 2 * sizeof(unsigned long long int),
                cudaHostAllocDefault);
  cudaHostAlloc((void **)&L.iiLimitsCT_m, 2 * sizeof(unsigned long long int),
                cudaHostAllocDefault);

  cudaHostGetDevicePointer((void **)&L.iiLimitsAT_d, (void *)L.iiLimitsAT_m, 0);
  cudaHostGetDevicePointer((void **)&L.iiLimitsALT_d, (void *)L.iiLimitsALT_m,
                           0);
  cudaHostGetDevicePointer((void **)&L.iiLimitsART_d, (void *)L.iiLimitsART_m,
                           0);
...
__host__ void Copy_Line(Line &L, Molecule &m, int NL) {
  ...
  cudaMemcpy(L.ialphaD_d, L.ialphaD_h, NL * sizeof(double),
